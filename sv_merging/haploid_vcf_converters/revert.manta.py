...
def convert_ins(var):
    var.ref='N'
    if var.info['SVTYPE_ORIG']=="DUP":
        var.alt='<DUP>'
        var.info['SVTYPE']="DUP"
        var.info['STRANDS']='-+:'+str(var.info['SU'])
        var.info['SVLEN']=var.info['INSLEN_ORIG']
        var.info['END']=var.pos+int(var.info['SVLEN'])
    elif var.info['SVTYPE_ORIG']=="INS":
        var.alt='<DUP>'
        var.info['SVTYPE']="DUP"
        var.info['STRANDS']='-+:'+str(var.info['SU'])
        var.info['SVLEN']=var.info['INSLEN_ORIG']
  ...
  with su.InputStream(args.manta_vcf) as input_stream:
    for line in input_stream:
      if in_header:
        ...
        re.sub('ill_ill', 'ill.diplid', ll[2])
        #ll[2]=args.caller+'_'+ll[2]
        v = Variant(ll, vcf)
        if v.get_info('SVTYPE') in ['INS', 'DUP']:
