...
def convert_variant(v,  chrdict):
    set_read_counts(v)
    set_cis_prs(v)
    if v.get_info('SVTYPE')=='INV':
      convert_inv(v)
    elif v.get_info('SVTYPE')=='DEL':
      convert_del(v)
...
def convert_del(var):
    ...
    var.info['END']=var.pos+len(var.ref)
    var.ref='N'
...
def convert_inv(var):
    ...
    var.info['END']=var.pos+int(var.info['SVLEN'])
    var.info['SVTYPE']='INV'
  ...
  with su.InputStream(args.manta_vcf) as input_stream:
    for line in input_stream:
      if in_header:
        ...
        ll[2]=args.caller+'.'+args.parent+'_'+ll[2]
        v = Variant(ll, vcf)
        if  v.get_info('SVTYPE')=='INV' or  v.get_info('SVTYPE')=='INS' or  v.get_info('SVTYPE')=='DEL' :
          if abs(int(v.get_info('SVLEN')))>50:
            gt=ll[9].split('|')[int(args.haplotype)-1];
            if gt=='1':
