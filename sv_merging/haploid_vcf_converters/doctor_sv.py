...
def convert_variant(vcf,  ll):
  svtype=ll[6].split('.')[0]
  v=convert_del(vcf, ll)
  if svtype=="DUP":
    v=convert_dup(vcf, ll)
  elif svtype=="INV":
    v=convert_inv(vcf, ll)
  elif svtype=='BND':
    v=convert_bnd(vcf, ll)
  elif svtype=='INS':
    v=convert_dup(vcf, ll)
  return v
...
def convert_del(vcf, ll):
    var = Variant([ll[0], ll[1], 'sv_'+ll[6], 'N', '<DEL>', 120, 'PASS',  'SVTYPE=DEL', 'GT', '0/1'], vcf)
...
def convert_dup(vcf, ll):
    var = Variant([ll[0], ll[1], 'sv_'+ll[6], 'N', '<INS>', 120, 'PASS',  'SVTYPE=INS', 'GT', '0/1'], vcf)
    var.info['STRANDS']='+.:'+str(4)
    var.info['INSLEN_ORIG']=(int(ll[5])-int(ll[1]))
    var.info['END']=int(ll[2])+2
    var.info['SVLEN']=2
    set_read_counts(var)
    set_cis_prs(var)
    return var
    
def convert_inv(vcf, ll):
    var = Variant([ll[0], ll[1], 'sv_'+ll[6], 'N', '<INV>', 120, 'PASS',  'SVTYPE=INV', 'GT', '0/1'], vcf)
    strands='++:2,--:2'
...
def convert_bnd(vcf, ll):
    alt='N]'+ll[3]+':'+str(ll[4])+']'
    strands='++:4'
    ref='N'
    var = Variant([ll[0], ll[1], 'sv_'+ll[6], ref, alt, 120, 'PASS', 'SVTYPE=BND', 'GT', '0/1'], vcf)
    var.info['STRANDS']=strands
    set_read_counts(var)
    set_cis_prs(var)
    return var
