...
def convert_variant(v,  chrdict):
     set_read_counts(v)
     set_cis_prs(v)
     if v.get_info('SVTYPE')=='DEL' or v.get_info('SVTYPE')=="DEL/INV":
         convert_del(v)
     elif v.get_info('SVTYPE')=='DUP' or v.get_info('SVTYPE')=='DUP/INS' or v.get_info('SVTYPE')=='INVDUP' or v.get_info('SVTYPE')=="INV/INVDUP" or v.get_info('SVTYPE')=="DUP:INT" or v.get_info('SVTYPE')=='DUP:TANDEM':
         convert_dup(v)
    ...
    for line in input_stream:
      if in_header:
        ...
        if v.get_info('SVTYPE')!='cnv':
          ...
          if svlen>=50:
            gt=ll[9].split(':')[0].split('/')
            #hh=ll[9].split('/[|:]/')
            if gt[int(args.haplotype)-1]=='1':
