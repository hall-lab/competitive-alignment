...
def add_arguments_to_parser(parser):
    ...
    parser.add_argument('-b', '--sv_bedpe', dest='sv_bedpe', required=True, help='sv bedpe')
    parser.add_argument('-p', '--haplotype', dest='haplotype', required=True)
    parser.add_argument('-r', '--parent', dest='parent', required=True)
...
def convert_variant(vcf,  ll, hap):
  svtype=ll[10]
  #svtype=ll[6].split('.')[0]
  v=convert_del(vcf, ll, hap)
  if svtype=="DUP":
    v=convert_dup(vcf, ll, hap)
  elif svtype=="INV":
    v=convert_inv(vcf, ll, hap)
  elif svtype=='BND':
    v=convert_bnd(vcf, ll, hap)
  elif svtype=='INS':
    v=convert_ins(vcf, ll, hap)
  return v
...
def convert_del(vcf, ll, hap):
    var = Variant([ll[0], ll[1], 'sv.'+hap+'_'+ll[6], 'N', '<DEL>', 120, 'PASS',  'SVTYPE=DEL', 'GT', '0/1'], vcf)
    var.info['STRANDS']='+-:'+str(4)
    var.info['SVLEN']=-(int(ll[5])-int(ll[1]))
    var.info['END']=ll[5]
    set_read_counts(var)
    set_cis_prs(var)
    return var

def convert_dup(vcf, ll, hap):
    var = Variant([ll[0], ll[1], 'sv.'+hap+'_'+ll[6], 'N', '<DUP>', 120, 'PASS',  'SVTYPE=DUP', 'GT', '0/1'], vcf)
    var.info['STRANDS']='-+:'+str(4)
    #var.info['INSLEN_ORIG']=(int(ll[5])-int(ll[1]))
    svlen=int(ll[5])-int(ll[1])
    var.info['SVLEN']=svlen
    var.info['END']=int(ll[2])+svlen
    var.info['SVTYPE_ORIG']='DUP'
    set_read_counts(var)
    set_cis_prs(var)
    return var

def convert_ins(vcf, ll, hap):
    var = Variant([ll[0], ll[1], 'sv.'+hap+'_'+ll[6], 'N', '<DUP>', 120, 'PASS',  'SVTYPE=DUP', 'GT', '0/1'], vcf)
    var.info['STRANDS']='-+:'+str(4)
    #var.info['INSLEN_ORIG']=(int(ll[5])-int(ll[1]))
    svlen=int(ll[5])-int(ll[1])
...
def convert_inv(vcf, ll, hap):
    var = Variant([ll[0], ll[1], 'sv.'+hap+'_'+ll[6], 'N', '<INV>', 120, 'PASS',  'SVTYPE=INV', 'GT', '0/1'], vcf)
    strands='++:2,--:2'
...
def convert_bnd(vcf, ll, hap):
    alt='N]'+ll[3]+':'+str(ll[4])+']'
    strands='++:4'
    ref='N'
    var = Variant([ll[0], ll[1], 'sv.'+hap+'_'+ll[6], ref, alt, 120, 'PASS', 'SVTYPE=BND', 'GT', '0/1'], vcf)
    var.info['STRANDS']=strands
    set_read_counts(var)
    set_cis_prs(var)
    return var
    ...
    for line in input_stream:
      if in_header:
        ...
        if line[0:6] == '#CHROM':
          ...
          vcf.add_info('SVLEN', '.', 'Integer', 'Difference in length between REF and ALT alleles')
          vcf.add_info('END', '1', 'Integer', 'End position of the variant described in this record')
          ...
          break
  with open(args.sv_bedpe, 'r') as bedpe_in:
    for line in bedpe_in:
      ll=line.rstrip().split('\t')
      svtype=ll[10]
      #svtype=ll[6].split('.')[0]
      svlen=abs(int(ll[5])-int(ll[2]))
      if svlen>50 and svtype in ['DEL', 'DUP', 'INS', 'INV', 'BND']:
        v=convert_variant(vcf, ll, args.parent)
